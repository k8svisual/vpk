...
export function svgIAM(data: any, fnum: any, k8cData: any) {
    let rectP1a: string = '<rect  x="0" y="0" width="';
    let rectP1b: string = '" height="';
    let rectP2: string =
        '" rx="5" stroke-dasharray="1, 2" stroke-width="1"  stroke="black" fill="#bfffda"/>' + '<text x="5" y="12" class="pickIcon">Security</text>';
    let rectH: number = 0;
    let rectW: number = 150;
    let rtn: string = '';
    ...
    if (typeof data.ServiceAccount !== 'undefined') {
        ...
        let nsKey: string = '0000-' + data.namespace;
        if (typeof k8cData[nsKey] !== 'undefined') {
            if (typeof data.ServiceAccount[0].name !== 'undefined') {
                let saName: string = data.ServiceAccount[0].name;
                if (typeof k8cData[nsKey].CRB !== 'undefined') {
                    let crb: any = k8cData[nsKey].CRB;
                    for (let c: number = 0; c < crb.length; c++) {
                        if (crb[c].subName === saName) {
                            let cFnum: any = crb[c].crbFnum;
                            rtn =
                                rtn +
