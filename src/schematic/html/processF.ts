...
export function processF(fnum: any, k8cData: any, countContainer: number, countInitContainer: number, crdRefCnt: number) {
    let genS: number = 0;
    let cfgS: number = 0;
    let iamS: number = 0;
    let podS: number = 0;
    ...
    let pvcH: number = 0;
    // let allH: number = 0;
    let outterName: string = 'No defined workload name';
    let html = '';
    let height = 0;

    // add the namespace and cluster icons
    let rtnHeader: any = svgHeader(k8cData[fnum], fnum, k8cData);
    if (rtnHeader.bnds.show === true) {
        html = html + '<g id="configH-' + fnum + '" transform="translate(5, -20)">' + rtnHeader.rtn + '</g>';
    }

    // config
    let rtnConfig: any = svgConfig(k8cData[fnum], fnum);
    ...
    // generators
    let rtnGen: any = svgGenerators(k8cData[fnum], fnum);
    ...
    // network
    let rtnNet: any = svgNetwork(k8cData[fnum], fnum);
    ...
    // security / IAM
    let rtnIAM: any = svgIAM(k8cData[fnum], fnum, k8cData);
    ...
    // PVC
    let rtnPvc: any = svgPVC(k8cData[fnum], fnum);
    ...
    let rtnPod: any = svgPod(k8cData[fnum], fnum, height, countContainer, countInitContainer);
