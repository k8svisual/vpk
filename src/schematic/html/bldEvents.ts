...
export function bldEvents(fnum: any, k8cData: any, html: string) {
    // build the table of Event messages
    if (typeof k8cData[fnum].Events !== 'undefined') {
        let evts: any[] = k8cData[fnum].Events;
        let hl: number = evts.length;
        let bgColor: string;
        if (hl > 0) {
            let msg: string;
            let evtHtml: string =
        ...
        if (hl > 0) {
            let evtMsg: string = '' + hl + ' events';
            let evtBtn: string =
                '<rect  x="200" y="90" width="75" height="75" rx="5" stroke-dasharray="1, 2" ' +
