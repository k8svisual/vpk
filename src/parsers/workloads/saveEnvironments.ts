 ...
 * @param kind kind
 */
export function saveEnv(obj: any, e: number, fnum: any, name: string, ns: string, kind: string): void {
    if (typeof obj[e].env === 'undefined') {
        return;
    }

    const secRef: Array<Record<string, string>> = [];
    const cmapRef: Array<Record<string, string>> = [];
