...
export function checkOwnerReferences(name: string, ns: string, fnum: any) {
    try {
        let oRef: any;
        let uid: string = vpk.yaml.metadata.uid;
        let kind: string = vpk.yaml.kind;
        let ownerId: string = 'self';
        let ownKind: string = 'self';
        let childAPI: string = vpk.yaml.apiVersion;
        let ownAPI: any;
